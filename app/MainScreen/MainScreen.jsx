...
const MainScreen = () => {
  ...
  useEffect(() => {
    const fetchEmployeeData = async () => {
      try {
        const response = await axios.get('https://makemyattendance.free.beeceptor.com/api/mm');
            ...
            <View style={styles.employeeData}>
              ...
              {empData ? (
                <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                  ...
                  <Text style={styles.designation}>{empData.designation || 'No Designation'}</Text>
                </View>
              ) : (
...
const styles = StyleSheet.create({
  ...
  employeeName: { textAlign: 'center', paddingTop: 10, fontSize: 18, fontWeight: '600' },
  designation: { textAlign: 'center', fontSize: 17, padding: 5 },
  InOut: {
    margin: 45, height: 100, width: 100, borderRadius: 65,
    justifyContent: 'center', alignItems: 'center', marginBottom: 30
  },
  InOutData: { fontSize: 20, color: 'white' },
  duty: { fontSize: 17, fontWeight: '600', paddingBottom: 20 },
  Check: { width: '90%', alignItems: 'flex-start', paddingLeft: 10, margin: 15 },
  intime: { fontSize: 16, margin: 10, fontWeight: '600' },
  checktime: { fontWeight: '500' },
  camera: {
    flex: 1, width: '100%', justifyContent: 'flex-end', alignItems: 'center', aspectRatio: 9 / 16
  },
  captureButton: { position: 'absolute', bottom: 20, left: '40%', marginLeft: -40, backgroundColor: 'transparent', padding: 10 }
});
